...
class ClipboardExporter:
    ...
    def generate_all_region_patterns(self, fmt: str = "hex") -> str:
        """Generate a separate pattern for each region.
        ...
        *fmt*: ``"hex"`` | ``"ascii"`` | ``"regex"``
        ...
        for start, end in regions:
            data = self._buffer.read(start, end - start + 1)
            counter = self._selection.next_yara_id()
            lines.append(self._format_one(data, counter, start, fmt))
        return "\n    ".join(lines)

    def build_wildcard_pattern_ascii(self) -> str:
        """Wildcard pattern using ASCII strings instead of hex."""
        return self._build_wildcard_fmt("ascii")

    def build_wildcard_pattern_regex(self) -> str:
        """Wildcard pattern using regex instead of hex."""
        return self._build_wildcard_fmt("regex")

    def _build_wildcard_fmt(self, fmt: str) -> str:
        """Wildcard pattern in the given format — hex gaps still use [N]."""
        if not self._buffer:
            return ""
        regions = self._selection.pattern_regions
        if not regions:
            return ""
        # Wildcard with [N] gaps only makes sense for hex
        # For ascii/regex, generate separate patterns per region
        return self.generate_all_region_patterns(fmt)
    ...
    @staticmethod
    def _escape_regex(data: bytes) -> str:
        _META = set(r"\.^$*+?{}[]|()")
        text = ""
        for b in data:
            ch = chr(b) if 0x20 <= b < 0x7F else ""
    ...
    def _format_one(self, data: bytes, counter: int, offset: int,
                    fmt: str) -> str:
        """Format a single pattern in the given style."""
        comment = f"  // 0x{offset:08X}, {len(data)} bytes"
        if fmt == "ascii":
            text = self._escape_ascii(data)
            return f'$str_{counter} = "{text}" ascii{comment}'
        elif fmt == "regex":
            text = self._escape_regex(data)
            return f"$re_{counter} = /{text}/{comment}"
        else:
            hex_str = _fmt_hex(data)
            return f"$hex_{counter} = {{ {hex_str} }}{comment}"
