...
class _ParseThread(QThread):
    """Background thread for PE/ELF parsing."""
    finished_results = Signal(object)  # PeInfo, ElfInfo, or None
    error = Signal(str)

    def __init__(self, buffer: HexDataBuffer, fmt: str):
        super().__init__()
        self._buffer = buffer
        self._fmt = fmt
        ...
        try:
            ...
            class Reader:
                ...
                def read(self, offset, length):
                    ...
                    return original.read(offset, length)
            reader = Reader()
            if "PE" in self._fmt or "MZ" in self._fmt:
                info = PeParser(reader).parse()
            elif "ELF" in self._fmt:
                info = ElfParser(reader).parse()
            else:
                info = None
            self.finished_results.emit(info)
...
class FormatViewerWidget(QWidget):
    ...
    def set_buffer(self, buf: HexDataBuffer):
        ...
        if not buf or buf.size() == 0:
            return
        self._fmt = buf.format_name
        if "PE" in self._fmt or "MZ" in self._fmt or "ELF" in self._fmt:
            self._label.setText(f"Parsing {self._fmt}...")
            self._progress.setVisible(True)
            self._thread = _ParseThread(buf, self._fmt)
            self._thread.finished_results.connect(self._on_parse_done)
            self._thread.error.connect(self._on_parse_error)
            retain_thread(self._thread)
            self._thread.start()
        else:
            self._label.setText(f"Format: {self._fmt} (no parser available)")

    def _on_parse_error(self, msg: str):
        if self.sender() is not self._thread:
            return
        self._progress.setVisible(False)
        self._label.setText(f"Parse failed: {msg}")
        self._thread = None

    def _on_parse_done(self, info):
        if self.sender() is not self._thread:
            return
        self._progress.setVisible(False)
        self._thread = None
        if info is None:
            self._label.setText("Parse failed")
            return
        if isinstance(info, PeInfo):
            self._populate_pe(info)
        elif isinstance(info, ElfInfo):
            self._populate_elf(info)

    # ── PE ──────────────────────────────────────────────────────────

    def _populate_pe(self, info: PeInfo):
    ...
    def _populate_elf(self, info: ElfInfo):
