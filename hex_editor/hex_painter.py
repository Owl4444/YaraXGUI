...
class PaintContext:
    ...
    __slots__ = (
        ...
        "first_line", "visible_lines", "viewport_width", "horizontal_offset",
        "sel_min", "sel_max",
        # colours
    ...
    def __init__(self, *, buffer, layout, selection, font,
                 ...
                 gutter_mode="offset",
                 diff_offsets=None, diff_bg=None, horizontal_offset=0):
...
class TextNotepadPainter(BasePaintStrategy):
    ...
    def _line_info(self, line_idx, ctx):
        ...
        self._first_column = min(content_len, max(0, (ctx.horizontal_offset - text_start) // L.char_w))
        self._content_end = content_end
        self._line_end = line_end
        read_len = max(0, min(content_len, max_visual_cols + 1) - self._first_column)
        data = ctx.buffer.read(line_start + self._first_column, read_len) if read_len > 0 else b""
    ...
    def _paint_line_content(self, painter, ctx, y, offset, data, data_len):
        ...
        for j in range(data_len):
            byte_offset = offset + self._first_column + j
            ch = data[j]
            x = text_start + (self._first_column + j) * L.char_w
            char_rect = QRect(x, y, L.char_w, L.line_h)
        ...
        # _line_info already found this row's end; no scan through the entire
        # line index is needed for each visible row.
        if sel.cursor == self._content_end and sel.cursor < self._line_end:
            cx = text_start + (self._content_end - offset) * L.char_w
            if ctx.horizontal_offset - L.char_w <= cx <= ctx.viewport_width:
