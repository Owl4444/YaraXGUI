...
from PySide6.QtGui import (QFont, QFontMetrics, QPainter, QColor,
                           QKeyEvent, QMouseEvent, QAction, QFontInfo, QFontDatabase)
...
class HexWidget(QAbstractScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        self._zoom_wheel_remainder = 0.0
        self.zoom_in_action = self._zoom_action("Zoom In", ["Ctrl++", "Ctrl+="], lambda: self.zoom_font(1))
        self.zoom_out_action = self._zoom_action("Zoom Out", ["Ctrl+-"], lambda: self.zoom_font(-1))
        self.zoom_reset_action = self._zoom_action("Reset Font Zoom", ["Ctrl+0"], self.reset_font_zoom)
        self.setToolTip("Zoom font: Ctrl+mouse wheel or Ctrl++ / Ctrl+−. Reset: Ctrl+0.")
    ...
    def _zoom_action(self, title, shortcuts, callback):
    ...
    def _set_zoom_size(self, size, anchor=None):
        size = max(6.0, min(72.0, size))
        font = QFont(self._font)
        if font.pointSizeF() == size:
            return
        anchor = anchor if anchor is not None else self.viewport().rect().center()
        column = (self.horizontalScrollBar().value() + anchor.x()) / self._layout.char_w
        row = self.verticalScrollBar().value() + anchor.y() / self._layout.line_h
        font.setPointSizeF(size)
        # A zoom changes metrics, but must not reset the configured font size,
        # move the selection, rescan the buffer, or jump to an offscreen cursor.
        super().setFont(font)
        self._refresh_font_metrics()
        self.horizontalScrollBar().setValue(round(column * self._layout.char_w - anchor.x()))
        self.verticalScrollBar().setValue(round(row - anchor.y() / self._layout.line_h))

    def wheelEvent(self, event):
        if event.modifiers() & Qt.KeyboardModifier.ControlModifier:
            delta = (event.angleDelta().y() / 120.0 if not event.angleDelta().isNull()
                     else event.pixelDelta().y() / 40.0)
            self._zoom_wheel_remainder += delta
            steps = int(self._zoom_wheel_remainder)
            if steps:
                self._zoom_wheel_remainder -= steps
                self.zoom_font(steps, event.position())
    ...
    def changeEvent(self, event):
        ...
        if event.type() in (QEvent.Type.FontChange, QEvent.Type.ApplicationFontChange) and hasattr(self, '_layout'):
            self._refresh_font_metrics()

    def _refresh_font_metrics(self):
        self._font = self._grid_font(self.font())
        self._layout.update_metrics(QFontMetrics(self._font, self.viewport()))
        ...
        while off < size:
            ...
            while True:
                ...
                if j < 0:
                    break
                start = off + j + 1
                self._text_max_line_bytes = max(self._text_max_line_bytes, start - self._text_line_starts[-1])
                if start < size:
                    self._text_line_starts.append(start)
                i = j + 1
            off += n
        self._text_max_line_bytes = max(self._text_max_line_bytes, size - self._text_line_starts[-1])
    ...
    def _update_scrollbar(self):
        ...
        width = self._layout.total_width
        if self._text_mode:
            columns = self._layout.text_cols if self._text_escape_mode else self._text_max_line_bytes
            width = self._layout.offset_width + (columns + 2) * self._layout.char_w
        self.horizontalScrollBar().setRange(0, min(2**31 - 1, max(0, width - self.viewport().width())))
        self.horizontalScrollBar().setPageStep(self.viewport().width())
        self.horizontalScrollBar().setSingleStep(self._layout.char_w)
    ...
    def paintEvent(self, event):
        ...
        ctx = PaintContext(
            ...
            viewport_width=self.viewport().width() + self.horizontalScrollBar().value(),
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton:
            off = self._layout.offset_from_point(
                event.position().toPoint() + QPoint(self.horizontalScrollBar().value(), 0),
            ...
            if off >= 0:
                if self._text_mode:
                    ...
                    x = event.position().toPoint().x() + self.horizontalScrollBar().value()
    ...
    def keyPressEvent(self, event: QKeyEvent):
        # Keyboard layouts can report '+' with Shift or Keypad modifiers that
        # do not match QAction's plain Ctrl++ sequence. Handle those variants
        # here too, before the empty-buffer and byte-editing paths.
        ...
        if (modifiers & Qt.KeyboardModifier.ControlModifier
            ...
            if event.key() in (Qt.Key.Key_Plus, Qt.Key.Key_Equal, Qt.Key.Key_Minus, Qt.Key.Key_0):
                if event.key() == Qt.Key.Key_0:
                    ...
                    self.zoom_font(-1 if event.key() == Qt.Key.Key_Minus else 1)
                event.accept()
                return
        if not self._buffer or self._buffer.size() == 0:
    ...
    def send_all_regions_to_yara_editor(self, fmt: str = "hex"):
        patterns = self._exporter.generate_all_region_patterns(fmt)
        if patterns:
    ...
    def _ensure_visible(self, offset: int):
        ...
        self._ensure_column_visible(offset)

    def _ensure_column_visible(self, offset):
        layout = self._layout
        column = offset % layout.bytes_per_line(self._text_mode)
        if self._text_mode and not self._text_escape_mode:
            column = offset - self._text_line_starts[self._byte_to_text_line(offset)]
        if self._text_mode:
            x = layout.offset_width + (column + 1) * layout.char_w
            width = layout.char_w
        elif self._selection.focus_ascii:
            x = layout.ascii_start + column * layout.char_w
            width = layout.char_w
        else:
            x = layout.hex_start + (column * 3 + column // 8) * layout.char_w
            width = 2 * layout.char_w
        bar = self.horizontalScrollBar()
        if x < bar.value():
            bar.setValue(x)
        elif x + width > bar.value() + self.viewport().width():
            bar.setValue(x + width - self.viewport().width())
    ...
    def contextMenuEvent(self, event):
        ...
        regions_menu = menu.addMenu("Send Regions to YARA Editor")
        act_reg_hex = regions_menu.addAction("As Hex Patterns    { AA BB }")
        act_reg_hex.triggered.connect(lambda: self.send_all_regions_to_yara_editor("hex"))
        act_reg_ascii = regions_menu.addAction('As ASCII Strings    "text"')
        act_reg_ascii.triggered.connect(lambda: self.send_all_regions_to_yara_editor("ascii"))
        act_reg_regex = regions_menu.addAction("As Regex Patterns    /pattern/")
        act_reg_regex.triggered.connect(lambda: self.send_all_regions_to_yara_editor("regex"))
        ...
        regions_menu.setEnabled(has_regions)
