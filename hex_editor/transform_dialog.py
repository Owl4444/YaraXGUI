...
from yaraxgui.ui.searchable_combo import SearchableComboBox
...
class TransformDialog(QDialog):
    ...
    def __init__(self, has_selection: bool, region_count: int,
                 ...
                 edit_mode: bool = False, probe_complete: bool = True):
        super().__init__(parent)
        ...
        self.resize(min(1280, int(available.width() * .92)), min(900, int(available.height() * .9)))

        self._has_selection = has_selection
        self._region_count = region_count
        self._probe_bytes = probe_bytes
        self._probe_complete = probe_complete
        ...
        self._op_combo = SearchableComboBox()
        self._populate_ops()
        top_row.addWidget(self._op_combo, 1)
        add_btn = self._add_operation_btn = QPushButton("+ Add to Recipe")
        add_btn.setDefault(False)
        add_btn.setAutoDefault(False)
        add_btn.clicked.connect(self._on_add_step)
        self._op_combo.editTextChanged.connect(
            lambda: add_btn.setEnabled(self._op_combo.selected_data() is not None))
        top_row.addWidget(add_btn)
        layout.addLayout(top_row)

        # Keep recipe cards beside the previews so adding steps cannot consume
        # the before/after panes' vertical space.
        splitter = self._main_splitter = QSplitter(Qt.Orientation.Horizontal)
        splitter.setChildrenCollapsible(False)

        # Recipe area (scrollable)
        recipe_box = self._recipe_box = QGroupBox("Recipe (applied in order)")
        ...
        recipe_box.setMinimumWidth(240)
        ...
        mono.setPointSize(max(11, self.font().pointSize()))
        ...
        self._step_combo.setSizeAdjustPolicy(QComboBox.SizeAdjustPolicy.AdjustToMinimumContentsLengthWithIcon)
        self._step_combo.setMinimumContentsLength(12)
        ...
        tools = QHBoxLayout()
        self._expand_preview = QToolButton()
        self._expand_preview.setText("Expand preview")
        self._expand_preview.setCheckable(True)
        self._expand_preview.toggled.connect(self._toggle_preview_space)
        tools.addWidget(self._expand_preview)
        self._debug_toggle = QToolButton()
        self._debug_toggle.setText("Show debug log")
        self._debug_toggle.setCheckable(True)
        tools.addWidget(self._debug_toggle)
        for label, delta in (("A−", -1), ("A+", 1)):
            zoom = QToolButton()
            zoom.setText(label)
            zoom.setToolTip("Change the font size in both previews")
            zoom.clicked.connect(lambda checked=False, delta=delta: self._zoom_previews(delta))
        ...
        for prefix, title in (("in", "Input:"), ("out", "Output:")):
            pane = QWidget()
            ...
            setattr(self, f"_preview_{prefix}_label", label)
            setattr(self, f"_preview_{prefix}", editor)
            self._preview_splitter.addWidget(pane)
        self._preview_splitter.setStretchFactor(0, 1)
        self._preview_splitter.setStretchFactor(1, 1)
        preview_layout.addWidget(self._preview_splitter, 1)
        ...
        self._preview_debug.setPlaceholderText("print(...) output and transform errors appear here.")
        preview_layout.addWidget(self._preview_debug)
        self._preview_debug.hide()
        self._preview_debug_label.hide()
        self._debug_toggle.toggled.connect(self._preview_debug.setVisible)
        self._debug_toggle.toggled.connect(self._preview_debug_label.setVisible)
        self._preview_debug.textChanged.connect(self._show_debug_if_needed)
    ...
    def _toggle_preview_space(self, expanded):
        if expanded:
            self._recipe_split_sizes = self._main_splitter.sizes()
        self._recipe_box.setVisible(not expanded)
        self._expand_preview.setText("Show recipe" if expanded else "Expand preview")
        if not expanded:
            self._main_splitter.setSizes(getattr(self, '_recipe_split_sizes', [320, 850]))

    def _zoom_previews(self, delta):
        for view in (self._preview_in, self._preview_out):
            font = view.font()
            font.setPointSize(max(6, min(48, font.pointSize() + delta)))
            view.setFont(font)

    def _show_debug_if_needed(self):
        if self._preview_debug.toPlainText().strip():
    ...
    def _on_add_step(self):
        name = self._op_combo.selected_data()
        if not name:
            return
        spec = find_spec(name)
        if spec is None:
    ...
    def _compute_step_outputs(
        ...
        for i, step in enumerate(steps):
            ...
            if spec.requires_full_input and not complete_input:
                return outputs, i, (
                    "Preview unavailable: this operation needs the complete compressed "
                    "payload, but only a 4 KiB sample is available. Apply will process "
                    "the full selected scope and report any decompression error."
                )
            try:
    ...
    def _update_preview(self):
        ...
        needs_full_input = any(
            enabled and (spec := find_spec(step.spec_name)) is not None
            and spec.requires_full_input
            for step, enabled in zip(steps, enabled_flags)
        )
        # Show N bytes limits display, not the compressed stream fed to a decoder.
        probe = self._probe_bytes if needs_full_input else self._probe_bytes[:max_bytes]
        ...
        outputs, failed_at, err = self._compute_step_outputs(
            ...
            complete_input=self._probe_complete and len(probe) == len(self._probe_bytes),
        )
    ...
    def _on_apply(self):
        ...
        if not steps:
            ...
            return
        # Complete-stream decoders must be validated by the window on the actual
        # selected scope, never against a prefix or dummy bytes.
        requires_full_input = any(
            (spec := find_spec(step.spec_name)) is not None and spec.requires_full_input
            for step in steps
        )
        # Validate other recipes on probe bytes (or a dummy).
        probe = self._probe_bytes or b"test"
        try:
            if not requires_full_input:
