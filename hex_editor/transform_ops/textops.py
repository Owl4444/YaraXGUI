...
def _unescape(text: str) -> str:
    """Interpret C-style escape sequences (\\xNN, \\n, \\t, \\r, \\0, \\\\)."""
    # Replace \\xNN first
    result = _HEX_ESC_RE.sub(lambda m: chr(int(m.group(1), 16)), text)
    # Then simple escapes
    for esc, char in _SIMPLE_ESCAPES.items():
...
@register_transform(
    ...
    params=[
        ...
        TransformParam("find", "Find", "text",
                       placeholder="needle   or   AB\\x90CD   or   \\b\\w+\\b",
                       help="Supports C escapes: \\xNN, \\n, \\t, \\r, \\0, \\\\"),
        TransformParam("replace", "Replace", "text",
                       placeholder="replacement   or   \\x00   or   \\g<0>",
                       help="Supports C escapes: \\xNN, \\n, \\t, \\r, \\0, \\\\"),
...
@register_transform(
    ...
    params=[
        TransformParam("mode", "Mode", "choice",
                       choices=["remove all 0x00", "remove trailing 0x00"],
                       default="remove all 0x00"),
    ],
    help="Strip NULL (0x00) bytes from the data.",
)
def remove_nulls(data: bytes, params: dict) -> bytes:
    mode = (params.get("mode") or "remove all 0x00").strip().lower()
    if "trailing" in mode:
