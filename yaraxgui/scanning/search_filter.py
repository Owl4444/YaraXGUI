...
# Hidden per-row searchable text (e.g. full path + hashes), attached to the
# column-0 item so filtering can match data that has no visible column.
SEARCH_TEXT_ROLE = Qt.ItemDataRole.UserRole + 100
    ...
    def filterAcceptsRow(self, source_row, source_parent):
        ...
        for col in range(model.columnCount()):
            ...
            if data and self._filter_text in str(data).lower():
                return True
        # Hidden searchable text (full path, hashes, ...) on column 0 —
        # allows matching full or partial hashes that have no visible column.
        extra = model.data(model.index(source_row, 0, source_parent), SEARCH_TEXT_ROLE)
        if extra and self._filter_text in str(extra).lower():
            return True
        return False
